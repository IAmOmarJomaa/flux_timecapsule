...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    ...
    def cache_text_encoder_outputs_if_needed(
        ...
        if args.cache_text_encoder_outputs:
            if not args.lowram:
                ...
                vae.to("cpu")
                # CRITICAL FIX: Disable UNet offload to avoid "Cannot copy out of meta tensor" error
                # unet.to("cpu")
    ...
    def prepare_unet_with_accelerator(
        ...
        if args.gradient_checkpointing:
             logger.info("Enabling gradient checkpointing and input gradients for Flux LoRA training.")
             unet.train()
             # We must call this before passing to accelerator to ensure hooks are registered
